...
DF[,keyOnerun:=paste0(expirament,task,pc,fold,transTarg,transform,algomodel,seed)]
dim(DF)
uDF<-unique(DF,by = c("pearson2","pRMSE","keyOnerun"))
dim(uDF)
...
tt <- seq(-2, 2, by = 0.1)
tt
Fn(tt) # Fn is a 'simple' function {with values k/12}
summary(Fn)
...
```
```{r}
DF[,keyOneRecipe:=paste0(expirament,task,transTarg,transform,algomodel,fold)]

uDF[order(-spearman2)][order(-pRMSE)][order(-pq9TargRank)][order(-gainin30)]
DF[keyOneRecipe=="QSlink3QSlinks115_602expoTransSL.randomForest"][order(fold)]
```
