...
DF[,timesd:=sd(time),by=keyOneRecipe]
DF[,time:=mean(time),by=keyOneRecipe]
...
what do I want the reduction in upper confidence level to be for a given time taken?
```{r}

los<-c(.1,1,2,3,pi,10,30,400,3000,5233)
for(i in los){
  print(i)
print(t.test(c(-1,0,1),conf.level = .95-sqrt(i)/100)$co)
print(t.test(c(-1,0,1),conf.level = .95-log(i)/100)$co)
}
if(F){
for(i in los){
  print(i)
...
for(i in names(uDF)[1:9]){#i<-"spearman2"
  ...
  uDF[,paste0(i,"u") := ttm(get(i), osd, cl = cl, dsda = dsda), by=keyOneRecipe]
  uDF[,paste0(i,"l") := ttm(get(i), osd, u_l="l", cl = cl, dsda = dsda),  by=keyOneRecipe] 
